...
;description : concatenate an array of string src to one string in des.
;source      : Modern X86 Assembly Language Programming 2nd Edition
;build       : mkdir build && cd build && qmake .. && make
;use         : extern "C" size_t ConcatStrings_(char* des, size_t des_size, const char* const* src, size_t src_n);
...
;      rdx = src ptr
;      rcx = src_n
; out : rax = -1 : invalied dest_size
;             n >= 0 : length of concatenated string

; to get the procedure working, we need to 'convert' from Windows ABI to Linux ABI
; moving the contents of rdx, rcx, rsi and rdi to r8, r9, rdx and rcx respectively.
; r8 and r9 are not used by the caller and can be used safely as long as you copy
; first to r8 and r9.  If not the values of rdx and rcx are lost before use.
    mov     r8,rdx                  ;r8 = src ptr
    mov     r9,rcx                  ;r9 = src_n
    mov     rdx,rsi
    mov     rcx,rdi
...
Done:
    mov     rax,r10                 ;rax = des_index (final length)
