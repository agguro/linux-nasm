...
void SsePiHistogramTimed(void)
{
    ...
    {
        tt.Start();
        SsePiHistogramCpp(histograms[0], pixel_buffer, num_pixels);
        tt.Stop();
    ...
    {
        tt.Start();
        SsePiHistogram_(histograms[1], pixel_buffer, num_pixels);
        tt.Stop();
        et[i][1] = tt.GetElapsedTime() * et_scale;
    }

    const char* fn = "__SsePackedIntegerHistogramTimed.csv";
