...
os_main:
     ...
     mov       si, pem3
     call      loadFile     
     jc        filenotfound                 ; file wasn't found, so not loaded and will not be executed
     call      execute
     jmp       continue
...
DISK_ADDRESS_PACKET      diskAddressPacket,PEMCS,PEMIP
...
.compareend:
     ...
     je        nonbootable
                                     ; If so, we're at the end of the directory, move on
...
execute:
     ...
     mov       ax, PEMCS                     ; Set all segments to match where kernel is loaded
     mov       ds, ax                   
     mov       es, ax                   
     mov       fs, ax                   
     mov       gs, ax
     mov       ax, PEMSS               ; stacksegment at last useable segment in low RAM
     cli
     push      sp
     mov       ss, ax
     mov       sp, PEMSP               ; stackpointer at last word of stack
     sti
     clc
     cld
     xor       ax, ax
