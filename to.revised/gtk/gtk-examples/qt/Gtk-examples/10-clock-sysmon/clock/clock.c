...
gint Repaint (gpointer data)
{
    ...
    dRadians = (float)(now_tm->tm_sec * 3.14 / 30.0);
    ...
    dRadians = (float)((now_tm->tm_min * 3.14 / 30.0) + (3.14 * now_tm->tm_sec / 1800.0));

    /* --- Draw Minutes --- */
    gdk_draw_line (drawable, drawing_area->style->black_gc,
                   midx, midy,
                   midx+(gint) (0.7 * radius * sin ((float)(dRadians))),
                   midy-(gint) (0.7 * radius * cos ((float)(dRadians))));
