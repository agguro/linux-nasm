;name: fibrecursive.asm
;
;description: calculation of Fibonacci numbers with recursion

bits 64

%include "../../../../qmake/math/fibonacci/fibrecursive/fibrecursive.inc"
...
section .rodata
;read-only data
    usage:      db      "Recursive Fibonacci (recursion) - by Agguro - 2013",10,"Usage: fibrecursive n,where n must be an integer in [0..50]",10
    .length:    equ     $-usage
    sResult1:   db      "fib("
    .length:    equ     $-sResult1
    sResult2:   db      ")="
...
main:
    ...
    cmp     rax,2
    jne     ShowUsage
    dec     rax                             ;minus 1 for programname exclusion
    shl     rax,3                           ;multiply by 8
    add     rax,rsi                         ;pointer to argv[1]
    mov     rax,[rax]                       ;pointer to argument
    mov     rsi,rax                         ;stringpointer in rsi
    ...
    jl      ShowUsage
    cmp     rax,MAX_VALUE                   ;if RAX > 50 then to big
    jg      ShowUsage

    push    rax                             ;store n to display later
    push    rax                             ;store n to calculate fib(n)
    call    GetTime
...
;***********************************************************************************************
;calculate the fibonacci of a number in RAX.
...
Bits128ToDecimal:
    ...
    mov     rbp,rsp
    sub     rsp,24                  ;three help variables to store three 32 bit registers
