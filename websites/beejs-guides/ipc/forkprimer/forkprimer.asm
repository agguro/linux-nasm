; Name        : fork.asm
;
; Build       : nasm "-felf64" forkprimer.asm -l forkprimer.lst -o forkprimer.o
;               ld -s -melf_x86_64 -o forkprimer forkprimer.o
;
; Description : A demonstration on fork syscall based on an example from Beej's Guide to IPC.
...
Parent:
     ...
     syscall     wait4, r12, status, 0, 0       ; wait for child to terminate
     and         rax, rax
     js          Error.Wait
     ...
     ; WEXITSTATUS is also a macro in sys/wait.inc. If we use this macro then the next three
     ; instructions are WEXITSTATUS rax, qword[status]
     
     mov         rax, qword[status]
     and         rax, 0xFF00
     shr         rax, 8
     push        rax                           ; save exit status so that we can regain it via echo $? 
     call        Hex2Dec
