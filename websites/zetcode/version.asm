;name        : version.asm
;
;description : The program is restricted to show some data stored in the glib and gtk
;              shared libraries.
...
;
;build       : nasm -f elf64 -F dwarf -g version.asm -o version.o
;              ld -m elf_x86_64 -g --dynamic-linker /lib64/ld-linux-x86-64.so.2 -lc -lgtk-x11-2.0 -lgdk-x11-2.0 -lglib-2.0 version.o -o version
...
_start:
    ...
    mov     r15,params
    ;we're gonna loop through the parameters in params list until we got the null
    ;pointer at the end and we exit the program
.repeat:
    mov     rax,[r15]
    test    rax,rax
    jz      .exit
